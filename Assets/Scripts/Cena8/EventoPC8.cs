...
public class EventoPC8 : MonoBehaviour
{
  ...
  private StateController8 state;
  ...
  private void OnEnable()
  {
    ...
    state = controller.GetComponent<StateController8>();
  ...
  private void OnMouseDrag()
  {
    ...
    {
      PegarPosicaoNoMundo();
      resistor.GetComponent<Resistores8>().SetPosicao(touchPosWorld + offset);
      state.reduzir = true;
  ...
  {
    GameObject sombra = resistor.GetComponent<Resistores8>().GetSombra();
  ...
  {
    TMP_Text valor = circuitoUI.transform.GetChild(0).GetComponent<TMP_Text>();
    string texto = (resistor.GetComponent<Resistores8>().GetResistencia().ToString()) + " Ohm";
  ...
  private void SpawnPeca()
  {
    if (state.spawn == true)
    {
      ...
      pecaCriada.GetComponent<Resistores8>().SetCriador(resistor);
